...
#ifdef STK_TUNE1
enum ps_int_state { STK_PSINT_NORM = 0, STK_PSINT_FIN_DET, STK_PSINT_CT_DET };
#endif
...
static int32_t stk3x1x_init_all_reg(struct stk3x1x_data *ps_data)
{
	...
	ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_PSCTRL_REG,
						ps_data->psctrl_reg);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
	}
	ret = stk3x1x_i2c_smbus_write_byte_data(
	...
	ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_WAIT_REG,
						ps_data->wait_reg);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
				     ...
				     uint16_t thd_l)
{
	...
	val[1] = thd_l & 0x00FF;
	ret = stk3x1x_i2c_write_data(ps_data->client, STK_THDL1_ALS_REG, 2,
				     val);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
				     ...
				     uint16_t thd_h)
{
	...
	val[1] = thd_h & 0x00FF;
	ret = stk3x1x_i2c_write_data(ps_data->client, STK_THDH1_ALS_REG, 2,
				     val);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
				...
				uint8_t org_flag_reg, uint8_t clr)
{
	...
	ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_FLAG_REG,
						w_flag);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
...
{
	int ret;
	ret = stk3x1x_i2c_smbus_write_byte_data(ps_data->client, STK_GSCTRL_REG,
						state);
	if (ret < 0)
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, ret);
	return ret;
				  ...
				  uint8_t mode)
{
	...
	if (enable) {
		...
		ret = stk3x1x_i2c_smbus_write_byte_data(
			ps_data->client, STK_WAIT_REG, ps_data->wait_reg);
		if (ret < 0) {
			printk(KERN_ERR "%s: write i2c error\n", __func__);
			return ret;
		}
		...
		ret = stk3x1x_i2c_smbus_write_byte_data(
			ps_data->client, STK_INT_REG, ps_data->int_reg);
		if (ret < 0) {
			printk(KERN_ERR "%s: write i2c error\n", __func__);
			return ret;
		}
...
#ifdef CALI_PS_EVERY_TIME
		else if (ps_data->boot_cali == 1 &&
			 ps_data->ps_low_thd_boot < STK_PS_HIGHER_THDH) {
...
static int32_t stk3x1x_get_als_reading(struct stk3x1x_data *ps_data)
{
	...
	if (ps_data->p_wv_r_bd_with_co & 0b010) {
		if (als_data < STK_ALS_THRESHOLD &&
		    ps_data->als_code_last > 10000) {
			ir_data = stk3x1x_get_ir_reading(
				ps_data, STK_ALS_READ_IRS_IT_REDUCE);
#ifdef STK_IRS
			if (ir_data > 0)
...
static void stk_als_ir_get_corr(struct stk3x1x_data *ps_data, int32_t als)
{
	...
	if (ps_data->ir_code) {
		...
		if (als < STK_IRC_MAX_ALS_CODE && als > STK_IRC_MIN_ALS_CODE &&
			...
			als_comperator =
				als * STK_IRC_ALS_NUMERA / STK_IRC_ALS_DENOMI;
...
static int stk3x1x_chk_reg_valid(struct stk3x1x_data *ps_data)
{
	...
	err = stk3x1x_i2c_read_data(ps_data->client, STK_PSCTRL_REG, 9,
				    &value[0]);
	if (err < 0) {
		printk(KERN_ERR "%s: fail, ret=%d\n", __func__, err);
		return err;
	...
	if ((value[1] != ps_data->alsctrl_reg) &&
	    (value[1] != (ps_data->alsctrl_reg - STK_IRS_IT_REDUCE)) &&
				   ...
				   const char *buf, size_t size)
{
	...
	val[1] = offset & 0x00FF;
	ret = stk3x1x_i2c_write_data(ps_data->client, STK_DATA1_OFFSET_REG, 2,
				     val);
	if (ret < 0) {
		printk(KERN_ERR "%s: write i2c error\n", __func__);
		return ret;
...
static struct attribute *stk_ges_attrs[] = {
	&ges_enable_attribute.attr, &ges_code_attribute.attr,
	&ges_poll_attribute.attr,   &ges_recv_attribute.attr,
	&ges_send_attribute.attr,   NULL
};
...
static struct device_attribute ps_enable_aso_attribute = __ATTR(
	enableaso, 0664, stk_ps_enable_aso_show, stk_ps_enable_aso_store);
...
static int stk_ps_tune_zero_final(struct stk3x1x_data *ps_data)
{
	...
	ps_data->ps_high_thd_boot =
		ps_data->ps_stat_data[1] + ps_data->stk_ht_n_ct;
	ps_data->ps_low_thd_boot =
		ps_data->ps_stat_data[1] + ps_data->stk_lt_n_ct;
	/*Huaqin modify for psensor near/far threshold by chenyijun5 at 2018/02/11 end*/
	ps_data->ps_thd_h = ps_data->ps_high_thd_boot;
	ps_data->ps_thd_l = ps_data->ps_low_thd_boot;
#else
...
/*Huaqin modify for psensor near/far threshold by chenyijun5 at 2018/02/11 start*/
	...
	if ((ps_data->psi_set != 0)) {
		ret = stk_ps_val(ps_data);
		if (ret == 0) {
			ret = stk3x1x_i2c_read_data(ps_data->client, 0x11, 2,
						    &value[0]);
			if (ret < 0) {
				printk(KERN_ERR "%s fail, err=0x%x", __func__,
				       ret);
				return ret;
			}
			word_data = (value[0] << 8) | value[1];
			if (word_data > 0) {
				...
				/*Huaqin modify for psensor near/far threshold by chenyijun5 at 2018/02/27 end*/
				{
					...
					ps_data->ps_thd_l =
						word_data +
						ps_data->stk_lt_n_ct;
					stk3x1x_set_ps_thd_h(ps_data,
							     ps_data->ps_thd_h);
					stk3x1x_set_ps_thd_l(ps_data,
							     ps_data->ps_thd_l);
					printk(KERN_INFO
...
int stk_tune1_check_stable(int val[], int val_len, int criteria, int *imax,
			   int *imin)
{
	int aa;
	int max, min;
...
static int stk_tune1_get_ct(uint32_t ps)
{
	...
	// {
	if (stk_tune1_check_stable(ps_get_ct.ps_val, STK_PS_VAL_BUF, PS_NOISE,
				   &ps_max, &ps_min) == 1) {
...
static int stk_tune1_fin_det(struct stk3x1x_data *ps_data, uint32_t ps,
			     int32_t nf_flag)
{
	...
	if (nf_flag == 0) {
		if (ps >= STK_FIN_THD) {
			...
			printk("%s: fixed HT=%d,LT=%d\n", __func__,
			       STK_PS_HIGHER_THDH, STK_PS_HIGHER_THDL);
			return 1;
		}
	} else {
		if (ps_far_re_cali) {
			...
			if (ps < STK_FIN_THD) {
				ps_data->ps_thd_h =
					ps + ps_data->stk_ht_n_ct * 2;
				ps_data->ps_thd_l =
					ps + ps_data->stk_lt_n_ct * 2;
#ifdef STK_POLL_PS
				stk3x1x_set_ps_thd_h(ps_data,
						     ps_data->ps_thd_h);
				stk3x1x_set_ps_thd_l(ps_data,
...
static int stk_tune1_ps_int_handle(struct stk3x1x_data *ps_data,
				   uint32_t ps_reading)
{
	int ret;
	int cnt;
	...
	nf_state = (ps_reading >= ps_data->ps_thd_h) ?
			   0 :
			   (ps_reading <= ps_data->ps_thd_l ?
				    1 :
			      ...
			      uint32_t als_reading)
{
	...
	if (ps_data->ir_code) {
		if (als_reading < STK_IRC_MAX_ALS_CODE &&
		    ...
		    ps_data->ir_code > STK_IRC_MIN_IR_CODE) {
			als_comperator = als_reading * STK_IRC_ALS_NUMERA /
					 STK_IRC_ALS_DENOMI;
			if (ps_data->ir_code > als_comperator)
				ps_data->als_correct_factor =
					STK_IRC_ALS_CORREC;
...
/*Huaqin add for ps INT mode by chenyijun5 at 2018/03/29 start*/
		...
		if (near_far_state == 0) {
			if ((reading > (ps_data->psi + STK_H_PS)) &&
				...
				if ((err = stk3x1x_set_ps_thd_h(
					     ps_data, ps_data->ps_thd_h))) {
					printk(KERN_ERR
					       "write high thd error: %d\n",
					       err);
				}
				if ((err = stk3x1x_set_ps_thd_l(
...
static const struct dev_pm_ops stk3x1x_pm_ops = { SET_SYSTEM_SLEEP_PM_OPS(
	stk3x1x_suspend, stk3x1x_resume) };
...
static int stk3x1x_set_input_devices(struct stk3x1x_data *ps_data)
{
	...
	*/
	...
	err = sysfs_create_group(&ps_data->ges_input_dev->dev.kobj,
...
void psensor_int_config(int state, struct pinctrl *pin)
{
	...
	if (state) {
		...
		if (IS_ERR(set_state)) {
			pr_err("[stk3x1x error]  psensor find psensor_active error!");
			return;
		}
	} else {
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		plat_data = devm_kzalloc(&client->dev,
					 sizeof(struct stk3x1x_platform_data),
					 GFP_KERNEL);
		if (!plat_data) {
